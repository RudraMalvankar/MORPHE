...
import {
  BookOpen, Search, Globe, BookMarked,
...
export default function KnowledgeBasePage() {
  ...
  useEffect(() => {
    ...
    fetch(API + "/knowledge/publishers").then(r => r.json()).then(setPublishers);
    fetch(API + "/knowledge/citations").then(r => r.json()).then(setCitationStyles);
    fetch(API + "/knowledge/journals").then(r => r.json()).then(setJournals);
  }, []);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Publisher guidelines, citation styles, and journal metrics.
          </p>
        </div>
        {stats && (
      ...
      <div className="flex gap-2">
        {[
          { key: "publishers", label: "Publishers", icon: Globe },
          { key: "citations", label: "Citation Styles", icon: BookMarked },
          { key: "journals", label: "Journal Metrics", icon: BarChart3 },
        ].map(({ key, label, icon: Icon }) => (
          <button
            key={key}
            onClick={() => { setTab(key as any); setSelectedItem(null); }}
      ...
      {tab === "publishers" && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {publisherEntries.map(([key, data]: [string, any]) => (
            <div
              key={key}
              onClick={() => setSelectedItem({ type: "publisher", key, data })}
            ...
            >
              ...
              <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                <div className="flex justify-between">
                  <span>Abstract limit</span>
                  <span className="font-mono">{data.abstract_word_limit || "N/A"} words</span>
                </div>
                <div className="flex justify-between">
                  <span>Page limit</span>
                  <span className="font-mono">{data.page_limit || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Review process</span>
                  <span className="font-mono">{data.review_process || "N/A"}</span>
                </div>
              </div>
      ...
      {tab === "citations" && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {citationEntries.map(([key, data]: [string, any]) => (
            <div
              key={key}
              onClick={() => setSelectedItem({ type: "citation", key, data })}
              className={`bg-card border border-border rounded-xl p-5 cursor-pointer transition-all hover:shadow-md ${
                selectedItem?.key === key ? "ring-2 ring-primary" : ""
              }`}
            >
              ...
              <div className="mt-3 space-y-2 text-xs">
                <div>
                  <span className="text-muted-foreground font-mono">In-text: </span>
                  <code className="bg-muted px-1.5 py-0.5 rounded">{data.in_text_format}</code>
                </div>
                <div>
                  <span className="text-muted-foreground font-mono">Order: </span>
                  <span className="text-foreground">{data.order}</span>
                </div>
              </div>
              {data.rules && (
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {journals.map((j, i) => (
              <div
                key={i}
                onClick={() => setSelectedItem({ type: "journal", key: j.journal_name, data: j })}
                className={`bg-card border border-border rounded-xl p-5 cursor-pointer transition-all hover:shadow-md ${
      ...
      {selectedItem && (
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3">
            ...
            <button onClick={() => setSelectedItem(null)} className="text-xs text-muted-foreground hover:text-foreground">
              Close
            </button>
          </div>
