...
export default function CDMEditor({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const handleRefine = async () => {
    ...
    setAiSuggestion("");
    try {
      const result = await apiFetch("/api/v1/generation/refine", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground font-mono">CDM v{doc.version_id} | {doc.sections.length} sections</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-[200px_1fr] gap-4">
        <div className="space-y-1">
          {doc.sections.map((sec, i) => (
            <button key={sec.id} onClick={() => { setActiveSection(i); setAiSuggestion(""); }}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                i === activeSection ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted"
              }`}>
              {sec.title}
            </button>
          ))}
        ...
        <div className="space-y-4">
          {!previewMode ? (
              ...
              <div className="flex items-center justify-between border-b border-border pb-3">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={handleRefine} disabled={isRefining}>
                    {isRefining ? <RefreshCw className="h-4 w-4 animate-spin mr-1" /> : <Sparkles className="h-4 w-4 mr-1" />}
                    AI Refine
                  </Button>
                </div>
              </div>
              <textarea value={section.content} onChange={(e) => updateSection(e.target.value)}
            ...
            <div className="bg-card border border-border rounded-xl p-8 space-y-6">
              ...
              {doc.sections.filter((s) => s.content.trim()).map((sec) => (
                <div key={sec.id}>
                  <h2 className="text-xl font-semibold text-foreground border-b border-border pb-2 mb-3">{sec.title}</h2>
          ...
          {aiSuggestion && (
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 space-y-3">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => { updateSection(aiSuggestion); setAiSuggestion(""); }}>
